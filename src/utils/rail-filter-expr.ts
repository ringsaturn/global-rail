...
const NEVER: FilterSpecification = [
  "==",
  ["literal", 1],
  ["literal", 0],
] as FilterSpecification;

export function buildRailFilterExpr(
  f: RailFilterState,
  ...
  if (f.statuses.under_construction)
    statusConds.push(["get", "is_under_construction"]);
  ...
  if (statusConds.length < 4) {
    parts.push(
      statusConds.length === 1 ? statusConds[0] : ["any", ...statusConds],
    );
  }
