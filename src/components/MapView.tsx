...
const REMOTE_RAIL_PMTILES_URL = 'https://dataset.ringsaturn.me/pmtiles/global_rail.pmtiles'
const LOCAL_RAIL_PMTILES_PROXY = '/pmtiles/global_rail.pmtiles'
const RAIL_PMTILES_URL = import.meta.env.DEV ? LOCAL_RAIL_PMTILES_PROXY : REMOTE_RAIL_PMTILES_URL
...
const OVERTURE_ATTRIBUTION = 'Rail data: <a href="https://overturemaps.org/" target="_blank" rel="noopener noreferrer">Overture Maps</a>'
...
function protomapsStyleUrl(theme: ResolvedTheme, language: Language): string | null {
...
async function fetchFilteredStyle(theme: ResolvedTheme, language: Language): Promise<maplibregl.StyleSpecification> {
  ...
  style.layers = style.layers.filter(layer => !REMOVE_LAYER_IDS.has(layer.id))
  return style
}
...
function nameFromProperties(props: Record<string, unknown> | null | undefined, language: Language): string {
...
function routeDetail(props: Record<string, unknown> | null | undefined, language: Language): string | undefined {
...
function removeRailLayers(map: maplibregl.Map) {
  for (const layerId of [STATION_LABEL_LAYER_ID, STATION_LAYER_ID, RAIL_LAYER_ID]) {
    if (map.getLayer(layerId)) map.removeLayer(layerId)
  }
  if (map.getSource('rail')) map.removeSource('rail')
}

function addRailLayers(map: maplibregl.Map, theme: ResolvedTheme, language: Language, filter: RailFilterState) {
  ...
  map.addLayer({
    ...
    minzoom: 0,
    paint: {
      'line-color': ['coalesce', ['get', 'color'], colors.lineFallback] as maplibregl.ExpressionSpecification,
      'line-width': ['interpolate', ['linear'], ['zoom'], 1, 0.35, 5, 0.75, 9, 1.8, 13, 4] as maplibregl.ExpressionSpecification,
      'line-opacity': ['interpolate', ['linear'], ['zoom'], 1, 0.5, 6, 0.82, 11, 0.96] as maplibregl.ExpressionSpecification,
    },
  })
  ...
  map.addLayer({
    ...
    minzoom: 6,
    paint: {
      'circle-radius': ['interpolate', ['linear'], ['zoom'], 6, 1.5, 10, 3, 14, 5] as maplibregl.ExpressionSpecification,
  ...
  map.addLayer({
    ...
    layout: {
      ...
      'text-size': ['interpolate', ['linear'], ['zoom'], 10, 10, 14, 12] as maplibregl.ExpressionSpecification,
      'text-offset': [0, 1.15],
      'text-anchor': 'top',
      'text-max-width': 8,
...
function watchRailSourceReady(map: maplibregl.Map, onReady: () => void): () => void {
  let ready = false
...
export default function MapView({ theme, language }: Props) {
  ...
  const [railFilter, setRailFilterState] = useState<RailFilterState>(railFilterRef.current)
  ...
  return (
    <div className="map-wrapper">
      ...
      <RailFilterPanel filter={railFilter} language={language} onChange={setRailFilter} />
      {status === 'error' && <div className="map-error">{STATUS_TEXT[language].error}</div>}
      {tooltip && (
        <div className="rail-tooltip" style={{ left: tooltip.x + 12, top: tooltip.y - 8 }}>
