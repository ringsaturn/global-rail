...
const STATUS_KEYS = ['operational', 'under_construction', 'disused', 'abandoned'] as const
...
export default function RailFilterPanel({ filter, onChange, language }: Props) {
  ...
  function toggleClass(cls: RailClass) {
    onChange({ ...filter, classes: { ...filter.classes, [cls]: !filter.classes[cls] } })
  }

  function toggleStatus(key: keyof RailFilterState['statuses']) {
    onChange({ ...filter, statuses: { ...filter.statuses, [key]: !filter.statuses[key] } })
  }

  function toggleUsage(key: keyof RailFilterState['usages']) {
    onChange({ ...filter, usages: { ...filter.usages, [key]: !filter.usages[key] } })
  }

  function toggleAllClasses() {
    const allOn = ALL_RAIL_CLASSES.every(c => filter.classes[c])
    const classes = Object.fromEntries(ALL_RAIL_CLASSES.map(c => [c, !allOn])) as RailFilterState['classes']
        ...
        <div className="rail-filter-body">
          <div className="filter-section">
            <div className="filter-section-head">
              ...
              <button className="filter-all-btn" onClick={toggleAllClasses} type="button">
                {ALL_RAIL_CLASSES.every(c => filter.classes[c]) ? text.deselectAll : text.selectAll}
              </button>
            </div>
            {ALL_RAIL_CLASSES.map(cls => (
              <label key={cls} className="filter-row">
                <input type="checkbox" checked={filter.classes[cls]} onChange={() => toggleClass(cls)} />
          ...
          <div className="filter-section">
            ...
            {STATUS_KEYS.map(key => (
              <label key={key} className="filter-row">
                <input type="checkbox" checked={filter.statuses[key]} onChange={() => toggleStatus(key)} />
          ...
          <div className="filter-section">
            ...
            {USAGE_KEYS.map(key => (
              <label key={key} className="filter-row">
                <input type="checkbox" checked={filter.usages[key]} onChange={() => toggleUsage(key)} />
          ...
          {inactive > 0 && (
            <button className="filter-reset-btn" onClick={() => onChange(defaultRailFilter())} type="button">
              {text.reset}
            </button>
          )}
        </div>
